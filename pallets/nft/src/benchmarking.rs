...
benchmarks! {
	...
	mint {
		...
		let class_data = 123;
		let class_id = peak_nft::Pallet::<T>::create_class(&caller, class_metadata.clone(), class_data).unwrap_or_default();
	...
	transfer {
		...
		let token_data = TokenData { locked:false };
		let class_id = peak_nft::Pallet::<T>::create_class(&caller, class_metadata.clone(), class_data).unwrap_or_default();
		let token_id = peak_nft::Pallet::<T>::mint(&caller, class_id, class_metadata, token_data).unwrap_or_default();
