...
fn create_class_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let event = Event::pallet_ko_nft(crate::Event::NFTTokenClassCreated(ALICE, CLASS_ID));
...
fn mint_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let event = Event::pallet_ko_nft(crate::Event::NFTTokenMinted(ALICE, CLASS_ID, TEST_QUANTITY));
		assert_eq!(last_event(), event);
	});
}
...
fn transfer_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let event = Event::pallet_ko_nft(crate::Event::NFTTokenTransferred(ALICE, BOB, CLASS_ID, TOKEN_ID));
		assert_eq!(last_event(), event);
	});
}
...
fn burn_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let event = Event::pallet_ko_nft(crate::Event::NFTTokenBurned(ALICE, CLASS_ID, TOKEN_ID));
		assert_eq!(last_event(), event);
	});
}
