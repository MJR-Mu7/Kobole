...
pub type TokenIdOf<T> = <T as peak_nft::Config>::TokenId;
pub type ClassIdOf<T> = <T as peak_nft::Config>::ClassId;
...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + peak_nft::Config<ClassData = ClassData, TokenData = TokenData> {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_class(origin: OriginFor<T>, metadata: Vec<u8>, data: T::ClassData) -> DispatchResultWithPostInfo {
			...
			let class_id = peak_nft::Pallet::<T>::create_class(&sender, metadata, data)?;
		...
		pub fn mint(
			origin: OriginFor<T>,
			class_id: <T as peak_nft::Config>::ClassId,
			metadata: Vec<u8>,
			...
			let class_info = peak_nft::Pallet::<T>::classes(class_id).ok_or(Error::<T>::ClassNotFound)?;
			ensure!(sender == class_info.owner, Error::<T>::NoPermission);
			let mut data = token_data;
			data.locked = false;
			for _ in 0..quantity {
				peak_nft::Pallet::<T>::mint(&sender, class_id, metadata.clone(), data.clone())?;
			}
		...
		pub fn transfer(
			...
			let sender = ensure_signed(origin)?;
			let _class_info = peak_nft::Pallet::<T>::classes(token.0).ok_or(Error::<T>::ClassNotFound)?;
			let token_info = peak_nft::Pallet::<T>::tokens(token.0, token.1).ok_or(Error::<T>::TokenNotFound)?;
			...
			peak_nft::Pallet::<T>::transfer(&sender, &to, token)?;
		...
		pub fn burn(origin: OriginFor<T>, token: (T::ClassId, T::TokenId)) -> DispatchResultWithPostInfo {
			...
			peak_nft::Pallet::<T>::burn(&sender, token)?;
		...
		pub fn destroy_class(origin: OriginFor<T>, class_id: T::ClassId) -> DispatchResultWithPostInfo {
			...
			peak_nft::Pallet::<T>::destroy_class(&sender, class_id)?;
...
impl<T: Config> Pallet<T> {
	pub fn is_owner(account: &T::AccountId, token: (T::ClassId, T::TokenId)) -> bool {
		peak_nft::Pallet::<T>::is_owner(account, token)
	}
	...
	pub fn toggle_lock(account: &T::AccountId, token_id: (T::ClassId, T::TokenId)) -> DispatchResult {
		let _class_info = peak_nft::Pallet::<T>::classes(token_id.0).ok_or(Error::<T>::ClassNotFound)?;
		peak_nft::Tokens::<T>::mutate_exists(token_id.0, token_id.1, |token| -> DispatchResult {
